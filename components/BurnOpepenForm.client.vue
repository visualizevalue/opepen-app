        ...
        >
          <template #start="{ start }">
            <ChooseOpepenModal
              ...
              @select="$event => { select($event); start() }"
            />
...
const burnedImageURI = computed(() => burnedMetadata.value?.image?.replace('ipfs://', 'https://ipfs.vv.xyz/ipfs/'))
const burned = async (event) => {
  const logs = event.logs.map(log => decodeEventLog({
    abi: parseAbi([
